...
public class NotificationTransactions {
    ...
    public NotificationTransactions(JdbcTemplate jdbc, JsonMapper json) {
        this.jdbc = jdbc;
        this.json = json;
    }
    ...
    public void accept(EventEnvelope<Payload> event) {
        ...
        if ((!completed && !event.eventType().equals("OrderCancelled")) || !event.aggregateType().equals("Order") || event.schemaVersion() != 1
            ...
            throw new IllegalArgumentException("Invalid terminal order event");
        String hash;
        try {
            hash = HexFormat.of().formatHex(java.security.MessageDigest.getInstance("SHA-256").digest((event.eventType() + event.payload()).getBytes(java.nio.charset.StandardCharsets.UTF_8)));
        ...
        if (jdbc.update("INSERT INTO processed_event(event_id, request_hash) VALUES (?, ?) ON CONFLICT DO NOTHING", event.eventId(), hash) == 0) {
            if (!hash.equals(jdbc.queryForObject("SELECT request_hash FROM processed_event WHERE event_id = ?", String.class, event.eventId())))
                throw new IllegalArgumentException("Changed notification event");
            return;
        }
        ...
        if (inserted == 0) {
            if (!hash.equals(jdbc.queryForObject("SELECT request_hash FROM notification WHERE order_id = ?", String.class, event.aggregateId())))
                throw new IllegalArgumentException("Conflicting terminal notification");
            return;
        }
        var output = new EventEnvelope<>(UUID.randomUUID(), "CustomerNotified", event.correlationId(), event.eventId(), "Notification", event.aggregateId(), 1, Instant.now(), 1,
                com.synechisveltiosi.notificationservice.infrastructure.Telemetry.currentTraceparentOr(event.traceparent()), new Delivered(event.aggregateId(), event.payload().customerId(), status, "SIMULATED"));
