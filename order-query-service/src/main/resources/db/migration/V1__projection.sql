...
INSERT INTO projection_generation(state) VALUES ('ACTIVE');
...
INSERT INTO projection_control VALUES (true, 1, NULL);
...
CREATE FUNCTION protect_projection_history() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN RAISE EXCEPTION 'Projection input history is append-only'; END;
$$;
CREATE TRIGGER projection_history_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON projection_event
    FOR EACH STATEMENT EXECUTE FUNCTION protect_projection_history();
CREATE TRIGGER projection_inbox_immutable BEFORE UPDATE OR DELETE OR TRUNCATE ON projection_inbox
    FOR EACH STATEMENT EXECUTE FUNCTION protect_projection_history();
