...
public class ProjectionBuilder {
    ...
    public ProjectionBuilder(ProjectionCodec codec) {
        this.codec = codec;
    }
    ...
    public Optional<OrderView> build(UUID id, List<EventEnvelope<JsonNode>> history) {
        ...
        for (var event : history)
            streams.computeIfAbsent(event.aggregateType(), ignored -> new ArrayList<>()).add(event);
        ...
        for (var entry : streams.entrySet()) {
            ...
            long version = 0;
            var events = new ArrayList<EventEnvelope<JsonNode>>();
            for (var event : entry.getValue()) {
                if (event.aggregateVersion() != version + 1) break;
                events.add(event);
                version++;
            }
            applied.put(entry.getKey(), events);
        ...
        if (!first.eventType().equals("OrderCreated"))
            throw new IllegalArgumentException("Order stream must start with creation");
        ...
        for (var event : history) {
            ...
            if (customer != null && !created.customerId().toString().equals(customer.asString()))
                throw new IllegalArgumentException("Inconsistent projection customer");
        }
        String status = "PENDING";
        var notes = new ArrayList<OrderView.Note>();
        Instant updated = first.occurredAt();
        ...
        for (var event : order.subList(1, order.size())) {
            ...
            switch (event.eventType()) {
                ...
                case "OrderFactRecorded" -> {
                    ...
                    if (Set.of(QueryEvents.Fact.PaymentFailed, QueryEvents.Fact.ShipmentFailed, QueryEvents.Fact.PaymentRefunded).contains(fact))
                        status = "COMPENSATING";
                }
                case "OrderCompleted", "OrderCancelled" -> {
                    if (terminal) throw new IllegalArgumentException("Multiple terminal outcomes");
                    status = event.eventType().equals("OrderCompleted") ? "COMPLETED" : "CANCELLED";
                    terminal = true;
                }
        ...
        UUID providerReference = null, refundReference = null;
        boolean notified = false;
        for (var entry : applied.entrySet())
            if (!entry.getKey().equals("Order")) for (var event : entry.getValue()) {
                switch (event.eventType()) {
                    case "InventoryReserved" -> inventory = "RESERVED";
                    case "InventoryReservationFailed" -> inventory = "REJECTED";
                    case "InventoryReleased" -> {
                        if (!inventory.equals("RESERVED"))
                            throw new IllegalArgumentException("Release without reservation");
                        inventory = "RELEASED";
                    }
                    case "PaymentCompleted" -> {
                        payment = "COMPLETED";
                        providerReference = codec.payload(event, QueryEvents.Payment.class).providerReference();
                    }
                    case "PaymentFailed" -> payment = "FAILED";
                    case "PaymentRefunded" -> {
                        if (!payment.equals("COMPLETED"))
                            throw new IllegalArgumentException("Refund without completed charge");
                        refund = "REFUNDED";
                        refundReference = codec.payload(event, QueryEvents.Payment.class).refundReference();
                    }
                    case "ShipmentCreated" -> {
                        shipping = "CREATED";
                        tracking = codec.payload(event, QueryEvents.Shipment.class).trackingNumber();
                    }
                    case "ShipmentFailed" -> shipping = "FAILED";
                    case "CustomerNotified" -> {
                        var message = codec.payload(event, QueryEvents.Notification.class);
                        if (terminal && !status.equals(message.orderStatus()))
                            throw new IllegalArgumentException("Conflicting notification outcome");
                        notified = true;
                    }
