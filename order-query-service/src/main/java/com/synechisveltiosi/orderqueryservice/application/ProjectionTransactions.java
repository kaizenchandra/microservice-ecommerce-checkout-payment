...
public class ProjectionTransactions {
    private final JdbcTemplate jdbc; private final ProjectionCodec codec; private final ProjectionBuilder builder;
    public ProjectionTransactions(JdbcTemplate jdbc, ProjectionCodec codec, ProjectionBuilder builder) { this.jdbc = jdbc; this.codec = codec; this.builder = builder; }
    @Transactional
    public void accept(EventEnvelope<JsonNode> event) {
        ...
        if (!inbox.isEmpty()) {
            if (!hash.equals(inbox.getFirst().get("content_hash"))) throw new IllegalArgumentException("Event identity changed");
            return;
        }
        ...
        if (!prior.isEmpty()) {
            ...
            if (!hash.equals(row.get("content_hash"))) throw new IllegalArgumentException("Producer version changed content");
    ...
    public Rebuild startRebuild() {
        var control = jdbc.queryForMap("SELECT * FROM projection_control WHERE singleton FOR UPDATE");
        if (control.get("building_generation") != null) throw new ApiException(HttpStatus.CONFLICT, "REBUILD_ACTIVE", "A projection rebuild is already running");
    ...
    public boolean rebuildStep(int batchSize) {
        ...
        for (var row : rows) {
            cursor = ((Number) row.get("sequence")).longValue(); project(generation, (UUID) row.get("order_id"), cursor);
        }
    ...
    public void failRebuild(long generation, String error) {
        var control = jdbc.queryForMap("SELECT * FROM projection_control WHERE singleton FOR UPDATE");
        if (control.get("building_generation") == null || ((Number) control.get("building_generation")).longValue() != generation) return;
    ...
    public Long buildingGeneration() { return jdbc.queryForObject("SELECT building_generation FROM projection_control WHERE singleton", Long.class); }
    public record Generation(long id, String state, long lastSequence, String lastError) { }
    public record Status(long activeGeneration, Long buildingGeneration, long journalEvents, long visibleOrders, long bufferedEvents, List<Generation> recentGenerations) { }
