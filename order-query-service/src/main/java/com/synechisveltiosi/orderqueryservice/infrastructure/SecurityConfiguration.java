    ...
    SecurityFilterChain security(HttpSecurity http, @Value("${demo.auth.basic-enabled:false}") boolean basicEnabled) throws Exception {
        ...
        return http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/api/order-views/admin/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.GET, "/api/order-views", "/api/order-views/**").hasAnyRole("CUSTOMER", "ADMIN")
                        .requestMatchers("/api/order-views", "/api/order-views/**").hasRole("ADMIN")
