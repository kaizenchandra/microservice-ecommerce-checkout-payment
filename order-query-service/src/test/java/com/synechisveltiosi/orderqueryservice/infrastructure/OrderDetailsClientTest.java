...
class OrderDetailsClientTest {
    ...
    @Test
    void circuitOpensWithoutAffectingHealthyOwnersAndRecoversOnProbe() {
        endpoint("/api/payments/", 503, "{}");
        endpoint("/api/inventory/", 200, body());
        endpoint("/api/shipping/", 200, body());
        client(2000);
        ...
        var blocked = get();
        assertEquals(UNAVAILABLE, blocked.payment().availability());
        assertEquals(AVAILABLE, blocked.shipping().availability());
        assertEquals(before + 2, calls.get());
        server.removeContext("/api/payments/");
        endpoint("/api/payments/", 200, body());
    ...
    @Test
    void missingRecordsDoNotOpenCircuit() {
        endpoint("/api/payments/", 404, "{}");
        endpoint("/api/inventory/", 404, "{}");
        endpoint("/api/shipping/", 404, "{}");
        client(2000);
