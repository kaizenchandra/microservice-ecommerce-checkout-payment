...
import com.synechisveltiosi.orderqueryservice.infrastructure.*;
...
class ProjectionApiIT {
    ...
    static JdbcTemplate jdbc; static ProjectionTransactions projection; static ProjectionCodec codec; static OrderQueries queries;
    static TransactionTemplate tx; static String base;
    ...
    static void startContext(boolean rebuild) {
        ...
        jdbc = context.getBean(JdbcTemplate.class); projection = context.getBean(ProjectionTransactions.class); codec = context.getBean(ProjectionCodec.class);
        queries = context.getBean(OrderQueries.class); tx = new TransactionTemplate(context.getBean(PlatformTransactionManager.class));
    ...
    static String topic(EventEnvelope<?> event) { return switch (event.aggregateType()) { case "Order" -> "order.events"; case "InventoryReservation" -> "inventory.events"; case "Payment" -> "payment.events"; case "Shipment" -> "shipping.events"; default -> "notification.events"; }; }
    static void accept(EventEnvelope<?> event) { projection.accept(codec.sanitize(topic(event), event.aggregateId().toString(), codec.encode(event))); }
    static void rebuildAll() { for (int i = 0; i < 1000 && projection.rebuildStep(10); i++) { } assertNull(projection.buildingGeneration()); }
    static HttpResponse<String> request(String method, String path, String user) throws Exception {
        ...
        if (user != null) builder.header("Authorization", "Basic " + Base64.getEncoder().encodeToString((user + ":" + PASSWORD).getBytes(StandardCharsets.UTF_8)));
    ...
    @Test void buffersGapsAndIndependentProducerStreamsUntilMissingVersionsArrive() {
        var id = UUID.randomUUID(); var identity = Map.<String, Object>of("orderId", id, "customerId", CUSTOMER);
        ...
        assertEquals("PENDING", initial.status()); assertEquals(1, initial.orderVersion()); assertEquals("UNKNOWN", initial.paymentStatus());
        ...
        var intermediate = queries.get(id, CUSTOMER, false); assertEquals("REFUNDED", intermediate.refundStatus()); assertEquals("RELEASED", intermediate.inventoryStatus());
        int version = 2;
        for (String fact : List.of("InventoryReserved", "PaymentCompleted", "ShipmentFailed", "PaymentRefunded", "InventoryReleased")) accept(event(id, "OrderFactRecorded", version++, Map.of("fact", fact)));
        var done = queries.get(id, CUSTOMER, false); assertEquals("CANCELLED", done.status()); assertEquals(7, done.orderVersion());
        assertEquals(2L, done.sourceVersions().get("Payment")); assertEquals(2L, done.sourceVersions().get("InventoryReservation"));
    }
    @Test void deduplicatesAliasesAndRejectsChangedIdentityWithoutLeakingTokens() throws Exception {
        var id = UUID.randomUUID(); var original = created(id, CUSTOMER); accept(original); accept(original);
        var alias = new EventEnvelope<>(UUID.randomUUID(), original.eventType(), original.correlationId(), original.causationId(), original.aggregateType(), id,
                1, original.occurredAt(), 1, null, original.payload()); accept(alias);
        ...
        var modified = new HashMap<>(original.payload()); modified.put("customerId", OTHER);
    ...
    @Test void apiOwnershipPaginationAndRebuildPermissions() throws Exception {
        var id = UUID.randomUUID(); accept(created(id, CUSTOMER)); var other = UUID.randomUUID(); accept(created(other, OTHER));
        String path = "/api/order-views/" + id;
        assertEquals(401, request("GET", path, null).statusCode()); assertEquals(404, request("GET", path, OTHER.toString()).statusCode());
        ...
        assertEquals(409, request("POST", "/api/order-views/admin/rebuild", "admin").statusCode()); rebuildAll();
    }
    @Test void rebuildKeepsActiveReadsAndCatchesConcurrentLiveInputBeforeSwitch() throws Exception {
        var id = UUID.randomUUID(); accept(created(id, CUSTOMER)); long old = projection.status().activeGeneration();
        long building = projection.startRebuild().generation(); assertTrue(projection.rebuildStep(1));
        ...
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            var a = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); accept(note); return true; });
            var b = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); return projection.rebuildStep(1); });
            a.get(15, TimeUnit.SECONDS); b.get(15, TimeUnit.SECONDS);
        }
        var live = queries.get(id, CUSTOMER, false); assertEquals(2, live.orderVersion());
        rebuildAll(); assertEquals(building, projection.status().activeGeneration());
    ...
    @Test void failedRebuildLeavesActiveGenerationReadableAndCanBeReplaced() {
        var id = UUID.randomUUID(); accept(created(id, CUSTOMER)); long active = projection.status().activeGeneration();
        ...
        try { new ProjectionRebuildWorker(projection, 100).poll(); }
        finally { jdbc.execute("ALTER TABLE order_projection DROP CONSTRAINT test_rebuild_failure"); }
        assertEquals(active, projection.status().activeGeneration()); assertNull(projection.buildingGeneration());
        ...
        long next = projection.startRebuild().generation(); rebuildAll(); assertEquals(next, projection.status().activeGeneration());
    }
    @Test void inputAndProjectionCommitAtomicallyAndRejectInvalidEnvelope() {
        var id = UUID.randomUUID(); var created = created(id, CUSTOMER);
        assertThrows(IllegalStateException.class, () -> tx.executeWithoutResult(status -> { accept(created); throw new IllegalStateException("Crash before commit"); }));
    ...
    @Test void realKafkaConsumerProjectsAndRestartResumesIncompleteRebuild() throws Exception {
        ...
        while (jdbc.queryForObject("SELECT count(*) FROM projection_event WHERE order_id = ?", Integer.class, id) == 0 && System.nanoTime() < deadline) Thread.sleep(50);
        var before = queries.get(id, CUSTOMER, false); long build = projection.startRebuild().generation(); projection.rebuildStep(1);
        ...
        try {
            ...
            assertEquals(build, projection.status().activeGeneration()); assertEquals(before, queries.get(id, CUSTOMER, false));
