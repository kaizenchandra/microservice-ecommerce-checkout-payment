...
class SagaIT {
    ...
    @TempDir static Path logs;
    static String gateway;
    @BeforeAll static void start() throws Exception {
        DB.start(); KAFKA.start();
        var kafkaProperties = new Properties(); kafkaProperties.put("bootstrap.servers", KAFKA.getBootstrapServers());
    ...
    static String database(String service) { return "jdbc:postgresql://" + DB.getHost() + ":" + DB.getMappedPort(5432) + "/" + service + "_saga"; }
    static void startService(String module, String database) throws Exception {
        ...
        if (database != null) command.addAll(List.of("--spring.datasource.url=" + database(database), "--spring.datasource.username=" + DB.getUsername(), "--spring.datasource.password=" + DB.getPassword()));
        if (module.equals("api-gateway")) for (var entry : PORTS.entrySet()) if (!entry.getKey().equals("api-gateway"))
        ...
        Process process = new ProcessBuilder(command).redirectErrorStream(true).redirectOutput(log.toFile()).start(); PROCESSES.add(process);
    ...
    static String tail(Path log) throws Exception { String text = Files.readString(log); return text.substring(Math.max(0, text.length() - 12000)); }
    ...
    static JsonNode request(String method, String path, String user, Object body, int status) throws Exception {
        ...
        assertEquals(status, response.statusCode(), path + ": " + response.body()); return JSON.readTree(response.body());
    }
    ...
    static Order create(String token, String country, String postal, int stock) throws Exception {
        var product = UUID.randomUUID(); var order = UUID.randomUUID();
        ...
        request("POST", "/api/orders", "checkout", payload, 202); return new Order(order, product);
    }
    static JsonNode awaitOrder(Order order, String status) throws Exception {
        long deadline = System.nanoTime() + Duration.ofSeconds(60).toNanos(); JsonNode last = null;
        while (System.nanoTime() < deadline) {
            last = request("GET", "/api/orders/" + order.id(), CUSTOMER, null, 200);
            if (last.get("status").asString().equals(status)) return last;
            Thread.sleep(100);
        }
        fail("Order did not reach " + status + ": " + last + "; logs: " + logs); return null;
    }
    static long scalar(String service, String sql, UUID id) throws Exception {
        try (var connection = DriverManager.getConnection(database(service), DB.getUsername(), DB.getPassword()); var statement = connection.prepareStatement(sql)) {
            statement.setObject(1, id); try (var result = statement.executeQuery()) { result.next(); return result.getLong(1); }
        }
    }
    static void awaitNotification(Order order, String status) throws Exception {
        long deadline = System.nanoTime() + Duration.ofSeconds(20).toNanos();
        while (scalar("notification", "SELECT count(*) FROM notification WHERE order_id = ?", order.id()) == 0 && System.nanoTime() < deadline) Thread.sleep(100);
    ...
    @Test void successCompletesShipsAndNotifiesOnceDespiteRedelivery() throws Exception {
        var order = create("tok_success", "US", "12345", 2); awaitOrder(order, "COMPLETED"); awaitNotification(order, "COMPLETED");
        ...
        replayOutbox("order", order.id(), "order.events"); replayOutbox("inventory", order.id(), "inventory.events"); replayOutbox("payment", order.id(), "payment.events");
        Thread.sleep(1500);
    ...
    @Test void inventoryRejectionCancelsWithoutCharging() throws Exception {
        var order = create("tok_success", "US", "12345", 0); awaitOrder(order, "CANCELLED"); awaitNotification(order, "CANCELLED");
        assertEquals(0, scalar("payment", "SELECT count(*) FROM payment WHERE payment_id = ?", order.id()));
    }
    @Test void declinedPaymentReleasesStockBeforeCancellation() throws Exception {
        var order = create("tok_declined", "US", "12345", 1); awaitOrder(order, "CANCELLED"); awaitNotification(order, "CANCELLED");
    ...
    @Test void failedShipmentWaitsForRetriedRefundAndRelease() throws Exception {
        var order = create("tok_success", "ZZ", "REFUND-RETRY", 1);
        long deadline = System.nanoTime() + Duration.ofSeconds(40).toNanos(); boolean sawCompensating = false;
        while (System.nanoTime() < deadline) {
            String status = request("GET", "/api/orders/" + order.id(), CUSTOMER, null, 200).get("status").asString();
            if (status.equals("COMPENSATING")) { sawCompensating = true; break; }
            Thread.sleep(50);
        }
        assertTrue(sawCompensating); awaitOrder(order, "CANCELLED"); awaitNotification(order, "CANCELLED");
    ...
    @Test void responseLossDuringChargeAndRefundDoesNotDuplicateEffects() throws Exception {
        var order = create("tok_timeout", "ZZ", "REFUND-TIMEOUT", 1); awaitOrder(order, "CANCELLED"); awaitNotification(order, "CANCELLED");
        assertEquals(1, scalar("payment", "SELECT charge_count FROM provider_charge WHERE payment_id = ?", order.id()));
    ...
    static JsonNode awaitView(Order order, String status) throws Exception {
        ...
        fail("Projection did not catch up; logs: " + logs); return null;
    }
    @Test void queryProjectsActualSagaEventsAndRebuildsThroughGateway() throws Exception {
        var success = create("tok_success", "US", "12345", 1);
        var failed = create("tok_success", "ZZ", "12345", 1);
        awaitOrder(success, "COMPLETED"); awaitOrder(failed, "CANCELLED");
        ...
        while (request("GET", "/api/order-views/admin/projection", "admin", null, 200).get("activeGeneration").asLong() != generation) {
            assertTrue(System.nanoTime() < deadline, "Rebuild did not finish"); Thread.sleep(100);
        }
    ...
    static void replayOutbox(String service, UUID id, String topic) throws Exception {
        var properties = new Properties(); properties.put("bootstrap.servers", KAFKA.getBootstrapServers()); properties.put("acks", "all");
        try (var producer = new KafkaProducer<String, String>(properties, new StringSerializer(), new StringSerializer());
            ...
            statement.setObject(1, id); try (var result = statement.executeQuery()) {
                while (result.next()) producer.send(new ProducerRecord<>(topic, id.toString(), result.getString(1))).get(10, TimeUnit.SECONDS);
            }
        }
    }
}
