...
final class OtlpCapture implements AutoCloseable {
    ...
    OtlpCapture() throws Exception {
        ...
        server.createContext("/v1/traces", exchange -> {
            try {
                decode(exchange.getRequestBody().readAllBytes());
    ...
    static String hex(byte[] bytes, int field) {
        return HexFormat.of().formatHex(first(bytes, field));
    }

    static String string(byte[] bytes, int field) {
        return new String(first(bytes, field), StandardCharsets.UTF_8);
    }

    static byte[] first(byte[] bytes, int field) {
        return values(bytes, field).stream().findFirst().orElse(new byte[0]);
    }

    static List<byte[]> values(byte[] bytes, int field) {
        return fields(bytes).stream().filter(f -> f.number() == field).map(Field::value).toList();
    }
    ...
    static List<Field> fields(byte[] data) {
        ...
        while (offset[0] < data.length) {
            ...
            switch (kind) {
                ...
                case 5 -> offset[0] += 4;
                case 2 -> {
                    int length = Math.toIntExact(varint(data, offset));
                    int end = Math.addExact(offset[0], length);
                    if (length < 0 || end > data.length) throw new IllegalArgumentException("Invalid protobuf length");
                    result.add(new Field((int) (tag >>> 3), Arrays.copyOfRange(data, offset[0], end)));
                    offset[0] = end;
                }
    ...
    void decode(byte[] request) {
        for (var resourceSpans : values(request, 1)) {
            ...
            for (var resource : values(resourceSpans, 1))
                for (var attribute : values(resource, 1))
                    if (string(attribute, 1).equals("service.name")) service = string(first(attribute, 2), 1);
            for (var scope : values(resourceSpans, 2))
                for (var span : values(scope, 2))
