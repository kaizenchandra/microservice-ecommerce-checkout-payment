    ...
    void restoresTraceAndLogContextAfterNestedWork() {
        ...
        try {
            try (var parent = telemetry.start("server", SpanKind.SERVER, "00-" + id + "-1234567890123456-01")) {
                ...
                try (var child = telemetry.start("child", SpanKind.INTERNAL, null)) {
                    assertNotEquals(server, Span.current().getSpanContext().getSpanId());
                }
                assertEquals(server, Span.current().getSpanContext().getSpanId());
            }
            assertEquals(previous, Span.current().getSpanContext());
            assertNull(MDC.get("traceId"));
            assertEquals("previous", MDC.get("correlationId"));
        } finally {
            MDC.clear();
            telemetry.close();
        }
