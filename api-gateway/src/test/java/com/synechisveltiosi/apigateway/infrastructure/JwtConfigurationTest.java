...
class JwtConfigurationTest {
    ...
    String encode(Map<String, Object> claims, String algorithm, String key) throws Exception {
        var base64 = Base64.getUrlEncoder().withoutPadding(); var json = JsonMapper.builder().build();
        ...
        var mac = javax.crypto.Mac.getInstance("HmacSHA256"); mac.init(new javax.crypto.spec.SecretKeySpec(key.getBytes(StandardCharsets.UTF_8), "HmacSHA256"));
        return input + "." + base64.encodeToString(mac.doFinal(input.getBytes(StandardCharsets.UTF_8)));
    }
    @Test void validCustomerMapsToItsSubjectAndRole() throws Exception {
        ...
        assertEquals(CUSTOMER, auth.getName()); assertTrue(auth.getAuthorities().stream().anyMatch(role -> role.getAuthority().equals("ROLE_CUSTOMER")));
    }
    @Test void rejectsMissingInvalidAndOverprivilegedClaims() throws Exception {
        var decoder = new JwtConfiguration().jwtDecoder(SECRET, "checkout-demo", "ecommerce-api");
        for (String name : List.of("aud", "iss", "sub", "roles", "iat", "exp")) {
            var claims = claims(); claims.remove(name);
            String encoded = encode(claims, "HS256", SECRET); assertThrows(JwtException.class, () -> decoder.decode(encoded), name);
        }
        for (var bad : Map.<String, Object>of("aud", List.of("wrong"), "iss", "wrong", "sub", "not-a-uuid", "roles", List.of("CUSTOMER", "ADMIN"),
            ...
            var claims = claims(); claims.put(bad.getKey(), bad.getValue()); String encoded = encode(claims, "HS256", SECRET);
            assertThrows(JwtException.class, () -> decoder.decode(encoded), bad.getKey());
        }
    }
    @Test void rejectsWrongSignatureAlgorithmAndWeakKey() throws Exception {
        var decoder = new JwtConfiguration().jwtDecoder(SECRET, "checkout-demo", "ecommerce-api");
        for (String algorithm : List.of("none", "HS512")) {
            String encoded = encode(claims(), algorithm, SECRET); assertThrows(JwtException.class, () -> decoder.decode(encoded));
        }
        String encoded = encode(claims(), "HS256", SECRET + "wrong"); assertThrows(JwtException.class, () -> decoder.decode(encoded));
