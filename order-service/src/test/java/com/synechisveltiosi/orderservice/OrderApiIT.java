...
class OrderApiIT {
    ...
    static CommandMetadata metadata() {
        return new CommandMetadata(UUID.randomUUID(), UUID.randomUUID(), null);
    }
    ...
    @Test
    void apiOwnershipValidationReplayAndIdempotency() throws Exception {
        var command = OrderTest.command(UUID.randomUUID());
        ...
        assertFalse(view.body().contains("paymentToken"));
    ...
    @Test
    void concurrentIdenticalCreatesAndExpectedVersionAppend() throws Exception {
        var command = OrderTest.command(UUID.randomUUID());
        var key = UUID.randomUUID();
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            var barrier = new CyclicBarrier(2);
            Callable<OrderDtos.Accepted> create = () -> {
                barrier.await(10, TimeUnit.SECONDS);
                return orders.create(command, key, metadata());
            };
            ...
            Callable<Boolean> append = () -> {
                try {
                    tx.executeWithoutResult(status -> {
                        var store = context.getBean(OrderEventStore.class);
                        var loaded = store.load(command.orderId());
                        try {
                            barrier.await(10, TimeUnit.SECONDS);
                        } catch (Exception e) {
                    ...
                    assertEquals("STALE_VERSION", conflict.code());
                    return false;
                }
            };
            var c = executor.submit(append);
            var d = executor.submit(append);
    ...
    @Test
    void rollbackIsAtomicAndHistoryCannotBeRewritten() {
        var id = UUID.randomUUID();
    ...
    @Test
    void publishesRealKafkaRecordsInStreamOrderWithStableIdentity() {
        var id = create();
        orders.addNote(id, OrderTest.CUSTOMER, new OrderDtos.AddNote(1L, "Second event"), metadata());
    ...
    void competingPollerSkipsLockedStreamWithoutOvertaking() throws Exception {
        var real = context.getBean(OutboxPublisher.class);
        for (int i = 0; i < 100 && real.publishOne(); i++) {
        }
        var id = create();
        orders.addNote(id, OrderTest.CUSTOMER, new OrderDtos.AddNote(1L, "Second"), metadata());
        var locked = new CountDownLatch(1);
        var release = new CountDownLatch(1);
        KafkaTemplate<String, String> kafka = mock(KafkaTemplate.class);
        when(kafka.send(any(ProducerRecord.class))).thenReturn(CompletableFuture.completedFuture(null));
        var publisher = new OutboxPublisher(jdbc, kafka, codec, context.getBean(MeterRegistry.class), context.getBean(com.synechisveltiosi.orderservice.infrastructure.Telemetry.class));
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            var holder = executor.submit(() -> tx.executeWithoutResult(status -> {
                ...
                try {
                    if (!release.await(15, TimeUnit.SECONDS))
                        throw new IllegalStateException("Timed out waiting for competing poller");
    ...
    @Test
    @SuppressWarnings("unchecked")
    void failedSendBlocksSuccessorAndAckBeforeRollbackPermitsDuplicate() {
