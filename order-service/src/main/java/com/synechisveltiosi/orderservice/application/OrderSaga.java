...
public class OrderSaga {
    ...
    public void accept(EventEnvelope<Payload> event) {
        ...
        if (event.schemaVersion() != 1 || !producer.equals(event.aggregateType()) || event.aggregateVersion() != expected
            ...
            throw new IllegalArgumentException("Invalid saga envelope");
        ...
        if (!order.snapshot().customerId().equals(event.payload().customerId()))
            throw new IllegalArgumentException("Wrong order customer");
        ...
        if (jdbc.update("INSERT INTO processed_saga_event(event_id, request_hash) VALUES (?, ?) ON CONFLICT DO NOTHING", event.eventId(), hash) == 0) {
            if (!hash.equals(jdbc.queryForObject("SELECT request_hash FROM processed_saga_event WHERE event_id = ?", String.class, event.eventId())))
        ...
        var metadata = new CommandMetadata(event.correlationId(), event.eventId(), com.synechisveltiosi.orderservice.infrastructure.Telemetry.currentTraceparentOr(event.traceparent()));
