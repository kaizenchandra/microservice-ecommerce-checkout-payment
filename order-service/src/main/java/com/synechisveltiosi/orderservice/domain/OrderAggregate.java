...
public final class OrderAggregate {
    ...
    private OrderAggregate(UUID id) {
        this.id = id;
    }

    public static OrderAggregate replay(UUID id, List<EventEnvelope<OrderEvents.Event>> history) {
        if (history.isEmpty()) {
            throw new IllegalArgumentException("An order needs an OrderCreated event");
        }
    ...
    private void apply(EventEnvelope<OrderEvents.Event> envelope) {
        ...
        switch (envelope.payload()) {
            case OrderEvents.OrderCreated event -> {
                if (created != null || version != 0 || !id.equals(event.orderId())) {
                    throw new IllegalStateException("Invalid order creation");
                }
                created = event;
                createdAt = envelope.occurredAt();
            }
            case OrderEvents.OrderFactRecorded event -> {
                if (created == null || terminal != null || facts.contains(event.fact()))
                    throw new IllegalStateException("Invalid order fact history");
    ...
    public OrderEvents.OrderNoteAdded addNote(String note) {
        if (notes.size() >= 20) {
            throw new IllegalArgumentException("An order may have at most 20 notes");
        }
    ...
    public boolean hasFact(OrderEvents.Fact fact) {
    ...
    public void validateFact(OrderEvents.Fact fact) {
        rejectContradiction(fact);
    }
    ...
    private void rejectContradiction(OrderEvents.Fact fact) {
        for (var pair : java.util.List.of(
            ...
            if (pair.contains(fact) && pair.stream().anyMatch(other -> other != fact && facts.contains(other)))
                throw new IllegalArgumentException("Contradictory saga outcome");
        }
    }

    public Status eligibleTerminal() {
        if (facts.contains(OrderEvents.Fact.InventoryReservationFailed)) return Status.CANCELLED;
        if (facts.containsAll(java.util.Set.of(OrderEvents.Fact.InventoryReserved, OrderEvents.Fact.PaymentFailed, OrderEvents.Fact.InventoryReleased)))
        ...
        if (facts.containsAll(java.util.Set.of(OrderEvents.Fact.InventoryReserved, OrderEvents.Fact.PaymentCompleted, OrderEvents.Fact.ShipmentCreated))
                && !facts.contains(OrderEvents.Fact.PaymentRefunded) && !facts.contains(OrderEvents.Fact.InventoryReleased))
            return Status.COMPLETED;
        return null;
    }

    public OrderEvents.OrderCreated snapshot() {
        return created;
    }
