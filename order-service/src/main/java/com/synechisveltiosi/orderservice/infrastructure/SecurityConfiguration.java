...
public class SecurityConfiguration {
    ...
    UserDetailsService users(PasswordEncoder encoder,
                             ...
                             @Value("${demo.auth.checkout-password}") String checkout) {
        return new InMemoryUserDetailsManager(
                ...
                User.withUsername("checkout").password(encoder.encode(checkout)).roles("CHECKOUT").build());
    }
    ...
    SecurityFilterChain security(HttpSecurity http, @Value("${demo.auth.basic-enabled:false}") boolean basicEnabled) throws Exception {
        ...
        return http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/api/orders/*/events", "/api/orders/*/outbox").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/orders").hasAnyRole("CHECKOUT", "ADMIN")
                        .requestMatchers(HttpMethod.POST, "/api/orders/*/notes").hasRole("CUSTOMER")
