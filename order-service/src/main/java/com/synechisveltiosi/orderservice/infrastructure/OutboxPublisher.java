...
public class OutboxPublisher {
    ...
    public OutboxPublisher(JdbcTemplate jdbc, KafkaTemplate<String, String> kafka, EventCodec codec, MeterRegistry metrics, Telemetry telemetry) {
        this.jdbc = jdbc;
        this.kafka = kafka;
        this.codec = codec;
    ...
    public boolean publishOne() {
        ...
        var event = codec.decode(row.payload());
        ...
        try (var correlation = MDC.putCloseable("correlationId", event.correlationId().toString());
            ...
            try (var trace = telemetry.start("kafka.publish", io.opentelemetry.api.trace.SpanKind.PRODUCER, event.traceparent())) {
                ...
                OrderService.afterCommit(() -> metrics.counter("outbox.publish.failures").increment());
            ...
            OrderService.afterCommit(() -> metrics.counter("outbox.published").increment());
