...
public class SagaListener {
    ...
    public SagaListener(JsonMapper json, OrderSaga saga) {
        this.json = json;
        this.saga = saga;
    }
    ...
    public void receive(ConsumerRecord<String, String> record) {
        ...
        String topic = switch (event.aggregateType()) {
            case "InventoryReservation" -> "inventory.events";
            case "Payment" -> "payment.events";
            case "Shipment" -> "shipping.events";
            default -> throw new IllegalArgumentException("Unsupported saga producer");
        };
        if (!topic.equals(record.topic()) || !event.aggregateId().toString().equals(record.key()))
            throw new IllegalArgumentException("Wrong saga topic or key");
        saga.accept(event);
    }
}
