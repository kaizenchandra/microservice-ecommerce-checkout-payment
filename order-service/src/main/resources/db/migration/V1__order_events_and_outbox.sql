...
END;
$$;
CREATE TRIGGER domain_event_append_only BEFORE UPDATE OR DELETE OR TRUNCATE ON domain_event
    FOR EACH STATEMENT EXECUTE FUNCTION reject_history_change();
CREATE TRIGGER order_command_append_only BEFORE UPDATE OR DELETE OR TRUNCATE ON order_command
    FOR EACH STATEMENT EXECUTE FUNCTION reject_history_change();

CREATE FUNCTION protect_outbox_identity() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
    IF ROW(NEW.id, NEW.aggregate_type, NEW.aggregate_id, NEW.aggregate_version, NEW.event_type, NEW.topic, NEW.payload, NEW.created_at)
