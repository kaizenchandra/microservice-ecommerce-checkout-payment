...
class Redrive {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try (var consumer = new KafkaConsumer<byte[], byte[]>(props, new ByteArrayDeserializer(), new ByteArrayDeserializer())) {
            consumer.assign(List.of(position));
            consumer.seek(position, offset);
            ...
            while (selected == null && System.nanoTime() < deadline) {
                for (var record : consumer.poll(Duration.ofMillis(250))) {
                    ...
                    if (record.offset() > offset)
                        throw new IllegalArgumentException("Requested offset is no longer retained");
                }
            }
        }
        ...
        if (args.length == 4) {
            System.out.println("Dry run; no record published. Add --execute after fixing the cause.");
            return;
        }
