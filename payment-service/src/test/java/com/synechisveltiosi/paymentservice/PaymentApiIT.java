...
import com.synechisveltiosi.paymentservice.infrastructure.InventoryEventListener;
...
class PaymentApiIT {
    ...
    static void startContext(boolean recovery) {
        ...
        payments = context.getBean(PaymentTransactions.class);
        worker = context.getBean(PaymentWorker.class);
        provider = context.getBean(PaymentProvider.class);
        ledger = context.getBean(ProviderLedger.class);
    ...
    static PaymentProvider.Request charge(EventEnvelope<PaymentEvents.InventoryReserved> e) {
        return new PaymentProvider.Request(e.aggregateId(), e.payload().total(), e.payload().paymentToken());
    }

    static PaymentDtos.View view(UUID id) {
        return payments.get(id, null, true);
    }
    ...
    @Test
    void successAndDeclineHaveOneDurableTerminalEvent() {
        for (String token : List.of("tok_success", "tok_declined")) {
            var event = event(token);
            payments.accept(event);
            assertEquals("PENDING", view(event.aggregateId()).status());
            ...
            payments.accept(event);
            payments.accept(redelivery(event));
            payments.retry(event.aggregateId());
    ...
    @Test
    void responseLossReconcilesWithoutChargingAgain() {
        var event = event("tok_timeout");
        payments.accept(event);
        assertTrue(worker.processOne());
    ...
    @Test
    void providerUnavailableIsUnknownUntilRecoveryAndNeverDeclined() {
        var event = event("tok_error");
        payments.accept(event);
        for (int i = 0; i < 2; i++) {
            assertTrue(worker.processOne());
            assertEquals("UNKNOWN", view(event.aggregateId()).status());
    ...
    @Test
    void concurrentAcceptsDeduplicateAndChangedInstructionsRollbackInbox() throws Exception {
        ...
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            var first = executor.submit(worker::processOne);
            var second = executor.submit(worker::processOne);
    ...
    @Test
    void expiredClaimsFenceStaleWorkersAndConcurrentProviderCallsHaveOneEffect() throws Exception {
        var event = event("tok_success");
        payments.accept(event);
        var old = payments.claim().orElseThrow();
        assertTrue(payments.claim().isEmpty());
        jdbc.update("UPDATE payment SET lease_until = now() - interval '1 second' WHERE payment_id = ?", event.aggregateId());
        var current = payments.claim().orElseThrow();
        assertNotEquals(old.token(), current.token());
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            var a = executor.submit(() -> provider.charge(charge(event)));
            var b = executor.submit(() -> provider.charge(charge(event)));
            var result = a.get(10, TimeUnit.SECONDS);
            assertEquals(result, b.get(10, TimeUnit.SECONDS));
            assertFalse(payments.finish(old, result));
            payments.defer(old, "StaleWorker");
            assertEquals("UNKNOWN", view(event.aggregateId()).status());
            assertTrue(payments.finish(current, result));
            assertFalse(payments.finish(old, result));
        }
        assertEquals(1, ledger.view(event.aggregateId()).orElseThrow().chargeCount());
        assertEquals(1, count("outbox_event", "aggregate_id", event.aggregateId()));
    ...
    @Test
    void providerCommitSurvivesBusinessRollbackAndOutboxRecoversAtomically() {
        var rolledBack = event("tok_success");
        assertThrows(IllegalStateException.class, () -> tx.executeWithoutResult(status -> {
            payments.accept(rolledBack);
            throw new IllegalStateException("Crash");
        }));
        assertEquals(0, count("payment", "payment_id", rolledBack.aggregateId()));
        assertEquals(0, count("processed_event", "event_id", rolledBack.eventId()));
        var event = event("tok_success");
        payments.accept(event);
        ...
        assertEquals("UNKNOWN", view(event.aggregateId()).status());
        assertEquals(0, view(event.aggregateId()).version());
        assertEquals(1, ledger.view(event.aggregateId()).orElseThrow().chargeCount());
        assertEquals(0, count("outbox_event", "aggregate_id", event.aggregateId()));
        payments.retry(event.aggregateId());
        assertTrue(worker.processOne());
    ...
    @Test
    void apiEnforcesOwnershipAndOmitsPrivatePaymentInstructions() throws Exception {
        var event = event("tok_success");
        payments.accept(event);
        String path = "/api/payments/" + event.aggregateId();
        ...
        var response = request("GET", path, CUSTOMER.toString());
        assertEquals(200, response.statusCode());
        assertFalse(response.body().contains("paymentToken"));
        assertFalse(response.body().contains("shippingAddress"));
        ...
        worker.processOne();
        assertEquals(200, request("GET", path + "/provider", "admin").statusCode());
    ...
    void realKafkaInputAndOutputPreserveIdentityAndRemoveFakeToken() throws Exception {
        ...
        while (count("payment", "payment_id", event.aggregateId()) == 0 && System.nanoTime() < deadline)
            Thread.sleep(50);
        assertEquals("PENDING", view(event.aggregateId()).status());
        worker.processOne();
        ...
        try (var consumer = new KafkaConsumer<String, String>(props, new StringDeserializer(), new StringDeserializer())) {
            ...
            while (received.size() < 2 && System.nanoTime() < deadline)
                for (var row : consumer.poll(Duration.ofMillis(200)))
                    if (event.aggregateId().toString().equals(row.key())) received.add(row);
            assertEquals(2, received.size());
            assertEquals(received.getFirst().value(), received.getLast().value());
            var found = received.getFirst();
            var output = json.readTree(found.value());
    ...
    @Test
    void invalidSchemaAndKeyCannotCreatePaymentIntents() {
        var event = event("tok_success");
        var listener = context.getBean(InventoryEventListener.class);
        ...
        assertEquals(0, count("payment", "payment_id", event.aggregateId()));
    }

    @Test
    void earlyShipmentFailureWaitsForChargeThenRefundsOnce() {
        var event = event("tok_success");
        var refunds = context.getBean(RefundTransactions.class);
        var refundWorker = context.getBean(RefundWorker.class);
        ...
        refunds.accept(failed);
        assertFalse(refundWorker.processOne());
        payments.accept(event);
        assertTrue(worker.processOne());
        assertTrue(refundWorker.processOne());
        refunds.accept(failed);
        assertFalse(refundWorker.processOne());
    ...
    @Test
    void applicationRestartAutomaticallyReconcilesLostProviderResponse() throws Exception {
        var event = event("tok_timeout");
        payments.accept(event);
        assertTrue(worker.processOne());
        assertEquals("UNKNOWN", view(event.aggregateId()).status());
        ...
        try {
            ...
            while (!view(event.aggregateId()).status().equals("COMPLETED") && System.nanoTime() < deadline)
                Thread.sleep(100);
            assertEquals("COMPLETED", view(event.aggregateId()).status());
            assertEquals(1, ledger.view(event.aggregateId()).orElseThrow().chargeCount());
            assertEquals(1, ledger.view(event.aggregateId()).orElseThrow().requests());
