...
CREATE FUNCTION protect_payment_input() RETURNS trigger
    LANGUAGE plpgsql AS $$
BEGIN
    IF
ROW(NEW.payment_id, NEW.customer_id, NEW.request_hash, NEW.input_event, NEW.created_at)
...
END IF;
    IF
OLD.status IN ('COMPLETED', 'FAILED') AND NEW IS DISTINCT FROM OLD THEN
        RAISE EXCEPTION 'Terminal charge outcome cannot be changed';
END IF;
RETURN NEW;
END;
$$;
CREATE TRIGGER payment_input_immutable
    BEFORE UPDATE
    ON payment
...
BEGIN
    IF
ROW(NEW.id, NEW.aggregate_id, NEW.aggregate_version, NEW.event_type, NEW.topic, NEW.payload, NEW.created_at)
