...
public class KafkaConfiguration {
    static final String DLT_SUFFIX = ".payment-service.DLT";

    @Bean
    KafkaAdmin.NewTopics deadLetterTopics() {
        return new KafkaAdmin.NewTopics(TopicBuilder.name("inventory.events" + DLT_SUFFIX).partitions(3).replicas(1).config("retention.ms", "604800000").build(),
                TopicBuilder.name("shipping.events" + DLT_SUFFIX).partitions(3).replicas(1).config("retention.ms", "604800000").build());
    }

    @Bean
    DefaultErrorHandler paymentErrorHandler(KafkaTemplate<String, String> kafka, MeterRegistry metrics) {
