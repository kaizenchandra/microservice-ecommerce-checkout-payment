...
public class RefundRecovery {
    ...
    public RefundRecovery(RefundWorker worker) { this.worker = worker; }
    @Scheduled(fixedDelayString = "${payment.recovery.poll-delay-ms:1000}")
    public void poll() {
        try { for (int i = 0; i < 10 && worker.processOne(); i++) { } }
        catch (Exception error) { org.slf4j.LoggerFactory.getLogger(getClass()).warn("Refund recovery retained ({})", error.getClass().getSimpleName()); }
    }
}
