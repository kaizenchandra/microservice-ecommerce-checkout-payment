...
public class RefundLedger {
    ...
    public RefundLedger(JdbcTemplate jdbc) { this.jdbc = jdbc; }
    ...
    public Attempt attempt(UUID paymentId, String postalCode) {
        if (!"COMPLETED".equals(jdbc.queryForObject("SELECT outcome FROM provider_charge WHERE payment_id = ?", String.class, paymentId))) throw new IllegalArgumentException("Refund needs a successful charge");
