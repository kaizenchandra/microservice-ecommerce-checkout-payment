...
public class RefundWorker {
    ...
    public RefundWorker(RefundTransactions refunds, RefundLedger ledger) {
    ...
    public boolean processOne() {
        if (org.springframework.transaction.support.TransactionSynchronizationManager.isActualTransactionActive())
            throw new IllegalStateException("Refund worker requires no business transaction");
        var pending = refunds.claim();
        ...
        try {
            var reference = ledger.lookup(claim.orderId()).orElseGet(() -> {
                ...
                if (result.responseLost() || result.reference() == null)
                    throw new IllegalStateException("Refund outcome unresolved");
                return result.reference();
            });
            refunds.finish(claim, reference);
        } catch (Exception error) {
            refunds.defer(claim, error.getClass().getSimpleName());
        }
        return true;
    }
}
