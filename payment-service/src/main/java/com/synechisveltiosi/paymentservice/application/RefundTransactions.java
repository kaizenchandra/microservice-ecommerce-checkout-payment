...
public class RefundTransactions {
    ...
    public record Input(UUID orderId, UUID customerId) { public Input { Objects.requireNonNull(orderId); Objects.requireNonNull(customerId); } }
    public record Claim(UUID orderId, UUID token, int attempts, UUID eventId, UUID customerId, UUID correlationId, String traceparent, PaymentEvents.InventoryReserved order) { }
    public record Refunded(UUID orderId, UUID paymentId, UUID customerId, com.synechisveltiosi.platform.contracts.Money total, UUID refundReference) { }
    private final JdbcTemplate jdbc; private final JsonMapper json;
    public RefundTransactions(JdbcTemplate jdbc, JsonMapper json) { this.jdbc = jdbc; this.json = json; }
    @Transactional
    public void accept(EventEnvelope<Input> event) {
        if (!event.eventType().equals("ShipmentFailed") || !event.aggregateType().equals("Shipment") || event.schemaVersion() != 1 || event.aggregateVersion() != 1
                || !event.aggregateId().equals(event.payload().orderId())) throw new IllegalArgumentException("Invalid shipment failure");
        int inserted = jdbc.update("INSERT INTO refund(order_id, event_id, customer_id, correlation_id, traceparent) VALUES (?, ?, ?, ?, ?) ON CONFLICT (order_id) DO NOTHING",
                event.aggregateId(), event.eventId(), event.payload().customerId(), event.correlationId(), event.traceparent());
        if (inserted == 0 && !event.payload().customerId().equals(jdbc.queryForObject("SELECT customer_id FROM refund WHERE order_id = ?", UUID.class, event.aggregateId()))) throw new IllegalArgumentException("Changed refund customer");
    }
    @Transactional
    public Optional<Claim> claim() {
        ...
        var row = rows.getFirst(); var id = (UUID) row.get("order_id"); var token = UUID.randomUUID();
        ...
        var order = json.readValue((String) row.get("input_event"), new TypeReference<EventEnvelope<PaymentEvents.InventoryReserved>>() { }).payload();
    ...
    public boolean finish(Claim claim, UUID reference) {
        ...
        var output = new EventEnvelope<>(UUID.randomUUID(), "PaymentRefunded", claim.correlationId(), claim.eventId(), "Payment", claim.orderId(), 2, Instant.now(), 1, claim.traceparent(),
