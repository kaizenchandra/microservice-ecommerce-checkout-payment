...
public class CompensationTransactions {
    ...
    private final JdbcTemplate jdbc; private final InventoryTransactions inventory;
    public CompensationTransactions(JdbcTemplate jdbc, InventoryTransactions inventory) { this.jdbc = jdbc; this.inventory = inventory; }
    @Transactional
    public void accept(EventEnvelope<Payload> event) {
        ...
        if ((!failed && !event.eventType().equals("PaymentRefunded")) || !event.aggregateType().equals("Payment") || event.schemaVersion() != 1
                || event.aggregateVersion() != (failed ? 1 : 2) || !event.aggregateId().equals(event.payload().orderId())) throw new IllegalArgumentException("Invalid compensation event");
        ...
        if (inserted == 0) {
            ...
            if (!event.payload().customerId().equals(existing.get("customer_id")) || !event.eventType().equals(existing.get("event_type"))) throw new IllegalArgumentException("Changed compensation instructions");
        }
    }
    @Transactional
    public boolean processOne() {
        ...
        var row = rows.getFirst(); var id = (UUID) row.get("order_id");
        var reservation = inventory.reservation(id);
        if (!reservation.customerId().equals(row.get("customer_id"))) throw new IllegalArgumentException("Wrong compensation customer");
