...
public class CompensationRecovery {
    ...
    public CompensationRecovery(CompensationTransactions transactions) {
    ...
    public void poll() {
        try {
            for (int i = 0; i < 10 && transactions.processOne(); i++) {
            }
        } catch (Exception error) {
            org.slf4j.LoggerFactory.getLogger(getClass()).warn("Inventory compensation retained ({})", error.getClass().getSimpleName());
        }
    }
}
