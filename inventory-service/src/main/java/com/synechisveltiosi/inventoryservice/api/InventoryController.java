...
public class InventoryController {
    ...
    public InventoryController(InventoryTransactions transactions, InventoryService inventory, OutboxPublisher outbox) {
        this.transactions = transactions; this.inventory = inventory; this.outbox = outbox;
    }
    @PostMapping("/stock")
    ...
    @GetMapping("/stock/{id}")
    public InventoryDtos.StockView stock(@PathVariable UUID id) { return transactions.getStock(id); }
    @PutMapping("/stock/{id}")
    public InventoryDtos.StockView set(@PathVariable UUID id, @Valid @RequestBody InventoryDtos.SetStock command) { return transactions.setStock(id, command); }
    @GetMapping("/reservations/{orderId}")
    public InventoryDtos.Reservation reservation(@PathVariable UUID orderId) { return transactions.reservation(orderId); }
