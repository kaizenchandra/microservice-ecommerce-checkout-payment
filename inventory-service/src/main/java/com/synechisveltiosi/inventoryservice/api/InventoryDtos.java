...
public final class InventoryDtos {
    ...
    public record SetStock(@NotNull @Min(0) @Max(1000000) Integer onHand,
    ...
    public record StockView(UUID productId, int onHand, int reserved, int available, long version) {
        public static StockView from(Stock stock) {
            return new StockView(stock.productId(), stock.onHand(), stock.reserved(), stock.available(), stock.version());
        }
    }

    public record Reservation(UUID orderId, UUID customerId, String status, long version, List<Line> items,
                              String reason) {
    }
}
