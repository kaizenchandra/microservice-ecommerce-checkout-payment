...
import com.synechisveltiosi.inventoryservice.application.*;
import com.synechisveltiosi.inventoryservice.domain.InventoryEvents;
import com.synechisveltiosi.inventoryservice.infrastructure.*;
...
class InventoryApiIT {
    ...
    @BeforeAll static void start() {
        ...
        inventory = context.getBean(InventoryService.class); transactions = context.getBean(InventoryTransactions.class);
    ...
    static UUID stock(int count) {
        var id = UUID.randomUUID(); transactions.createStock(new InventoryDtos.CreateStock(id, count)); return id;
    }
    ...
    @Test void securedStockApiValidatesVersionsAndPreservesReservations() throws Exception {
        String path = "/api/inventory/stock"; var command = new InventoryDtos.CreateStock(UUID.randomUUID(), 5);
        ...
        var event = event(command.productId(), 4); inventory.reserve(event);
    ...
    @Test void allOrNothingAndSemanticDeduplication() {
        var first = stock(5); var scarce = stock(0);
        ...
        var accepted = event(first, 2); inventory.reserve(accepted); inventory.reserve(accepted); inventory.reserve(redelivery(accepted));
        ...
        var missing = event(UUID.randomUUID(), 1); inventory.reserve(missing);
    ...
    @Test void lastUnitAndDuplicateOrdersUnderConcurrency() throws Exception {
        var product = stock(1); var first = event(product, 1); var second = event(product, 1);
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            var barrier = new CyclicBarrier(2);
            var a = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); inventory.reserve(first); return true; });
            var b = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); inventory.reserve(second); return true; });
            ...
            var c = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); inventory.reserve(duplicate); return true; });
            var d = executor.submit(() -> { barrier.await(10, TimeUnit.SECONDS); inventory.reserve(redelivery(duplicate)); return true; });
            c.get(15, TimeUnit.SECONDS); d.get(15, TimeUnit.SECONDS);
    ...
    @Test void versionConflictRollsBackAndOutboxFailureRollsBackWholeReservation() throws Exception {
        var product = stock(5); var repository = context.getBean(StockRepository.class); var barrier = new CyclicBarrier(2);
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            Callable<Boolean> change = () -> {
                try {
                    tx.executeWithoutResult(status -> {
                        ...
                        loaded.reserve(1); repository.flush();
                    }); return true;
                } catch (org.springframework.dao.OptimisticLockingFailureException conflict) { return false; }
            };
            var a = executor.submit(change); var b = executor.submit(change);
        ...
        try { assertThrows(org.springframework.dao.DataIntegrityViolationException.class, () -> inventory.reserve(event)); }
        finally { jdbc.execute("ALTER TABLE outbox_event DROP CONSTRAINT test_failure"); }
    ...
    @Test @SuppressWarnings("unchecked") void realOrderConsumerAndInventoryOutboxPreserveOrderingAndMetadata() throws Exception {
        ...
        while (count("reservation", "order_id", event.aggregateId()) == 0 && System.nanoTime() < deadline) Thread.sleep(50);
        assertEquals("RESERVED", transactions.reservation(event.aggregateId()).status());
        ...
        try (var consumer = new KafkaConsumer<String, String>(props, new StringDeserializer(), new StringDeserializer())) {
            consumer.subscribe(List.of("inventory.events")); var records = new ArrayList<ConsumerRecord<String, String>>();
            deadline = System.nanoTime() + Duration.ofSeconds(20).toNanos();
            while (records.size() < 2 && System.nanoTime() < deadline)
                for (var record : consumer.poll(Duration.ofMillis(200))) if (event.aggregateId().toString().equals(record.key())) records.add(record);
            assertEquals(2, records.size());
            var first = json.readTree(records.getFirst().value()); var second = json.readTree(records.getLast().value());
            assertEquals("InventoryReserved", first.get("eventType").asString());
            assertEquals("InventoryReleased", second.get("eventType").asString());
            assertEquals(1, first.get("aggregateVersion").asLong()); assertEquals(2, second.get("aggregateVersion").asLong());
    ...
    @Test void invalidIdentityAndSchemaNeverMarkProcessed() {
        var event = event(stock(2), 1); var listener = context.getBean(OrderEventListener.class);
    ...
    @Test @SuppressWarnings("unchecked") void failedPublicationBlocksSuccessorAndRollbackKeepsStableEventIdentity() {
        var real = context.getBean(OutboxPublisher.class);
        for (int i = 0; i < 100 && real.publishOne(); i++) { }
        var event = event(stock(4), 2); inventory.reserve(event);
        inventory.release(event.aggregateId(), UUID.randomUUID(), event.correlationId());
        ...
        var publisher = new OutboxPublisher(jdbc, kafka, json, context.getBean(io.micrometer.core.instrument.MeterRegistry.class));
