...
class KafkaRecoveryTest {
    @Test @SuppressWarnings("unchecked") void recoveryWaitsForAcknowledgementAndPreservesRecord() throws Exception {
        ...
        when(kafka.send(any(ProducerRecord.class))).thenAnswer(call -> {
            ...
            assertEquals("order.events.inventory-service.DLT", output.topic()); assertEquals(2, output.partition());
            assertEquals("identity", output.key()); assertEquals("original bytes", output.value());
            assertEquals(3, output.headers().toArray().length);
            sent.countDown(); return acknowledged;
        });
        ...
        try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
            ...
            assertTrue(sent.await(2, TimeUnit.SECONDS)); assertFalse(result.isDone());
            acknowledged.complete(null); result.get(2, TimeUnit.SECONDS);
            assertEquals(1, metrics.counter("consumer.dlt.published").count());
        } finally { metrics.close(); }
    }
    @Test @SuppressWarnings("unchecked") void failedAcknowledgementCannotRecoverSourceRecord() {
        var kafka = (KafkaTemplate<String, String>) mock(KafkaTemplate.class);
        ...
        try {
            ...
            assertFalse(error.getMessage().contains("private diagnostic")); assertNull(error.getCause());
            assertEquals(0, metrics.counter("consumer.dlt.published").count());
            assertEquals(1, metrics.counter("consumer.dlt.failures").count());
        } finally { metrics.close(); }
    }
    @Test @SuppressWarnings("unchecked") void errorHandlerDoesNotRecoverAfterFailedSend() {
        var kafka = (KafkaTemplate<String, String>) mock(KafkaTemplate.class);
