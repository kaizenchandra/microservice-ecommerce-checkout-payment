...
(
    ...
    status       VARCHAR(30) NOT NULL CHECK (status IN ('CREATED', 'FAILED')),
...
(
    id                UUID PRIMARY KEY,
    aggregate_id      UUID         NOT NULL REFERENCES shipping (order_id),
    aggregate_version BIGINT       NOT NULL CHECK (aggregate_version > 0),
    event_type        VARCHAR(100) NOT NULL,
    topic             VARCHAR(100) NOT NULL CHECK (topic = 'shipping.events'),
