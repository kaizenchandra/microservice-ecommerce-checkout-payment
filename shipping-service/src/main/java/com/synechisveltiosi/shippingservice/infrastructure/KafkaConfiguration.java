...
public class KafkaConfiguration {
    static final String DLT_SUFFIX = ".shipping-service.DLT";

    @Bean
    KafkaAdmin.NewTopics deadLetterTopics() {
        return new KafkaAdmin.NewTopics(TopicBuilder.name("payment.events" + DLT_SUFFIX).partitions(3).replicas(1).config("retention.ms", "604800000").build());
    }

    @Bean
    DefaultErrorHandler shippingErrorHandler(KafkaTemplate<String, String> kafka, MeterRegistry metrics) {
