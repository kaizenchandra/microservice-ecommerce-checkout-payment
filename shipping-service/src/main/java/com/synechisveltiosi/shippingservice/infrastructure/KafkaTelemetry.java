...
public class KafkaTelemetry implements RecordInterceptor<Object, Object> {
    ...
    public KafkaTelemetry(Telemetry telemetry, MeterRegistry metrics, JsonMapper json) {
    ...
    public ConsumerRecord<Object, Object> intercept(ConsumerRecord<Object, Object> record, Consumer<Object, Object> consumer) {
        ...
        if (parent == null) {
            try {
                ...
                var value = node.get("traceparent");
        ...
        active.set(trace);
        trace.attribute("messaging.destination.name", record.topic());
        try {
            var node = json.readTree((String) record.value());
            for (String key : new String[]{"correlationId", "eventId", "aggregateId"}) {
                if (node.get(key) != null)
                    MDC.put(key.equals("aggregateId") ? "orderId" : key, java.util.UUID.fromString(node.get(key).asString()).toString());
            }
            String type = node.get("eventType").asString();
            if (type.matches("[A-Za-z]{1,80}")) MDC.put("eventType", type);
    ...
    public void success(ConsumerRecord<Object, Object> record, Consumer<Object, Object> consumer) {
        metrics.counter("messaging.processed", "outcome", "success").increment();
        LOG.info("Kafka record processed");
    }

    @Override
    public void failure(ConsumerRecord<Object, Object> record, Exception error, Consumer<Object, Object> consumer) {
        if (active.get() != null) active.get().failed();
        metrics.counter("messaging.processed", "outcome", "failure").increment();
    }
