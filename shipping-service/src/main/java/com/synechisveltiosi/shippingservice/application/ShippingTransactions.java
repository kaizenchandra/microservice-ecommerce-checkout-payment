...
public class ShippingTransactions {
    ...
    public ShippingTransactions(JdbcTemplate jdbc, JsonMapper json) { this.jdbc = jdbc; this.json = json; }
    @Transactional
    public void accept(EventEnvelope<ShippingEvents.PaymentCompleted> event) {
        ...
        if (!event.eventType().equals("PaymentCompleted") || !event.aggregateType().equals("Payment") || event.schemaVersion() != 1
                || event.aggregateVersion() != 1 || !event.aggregateId().equals(input.orderId()) || !input.paymentId().equals(input.orderId())) throw new IllegalArgumentException("Invalid completed payment");
        ...
        if (jdbc.update("INSERT INTO processed_event(event_id, request_hash) VALUES (?, ?) ON CONFLICT DO NOTHING", event.eventId(), hash) == 0) {
            if (!hash.equals(jdbc.queryForObject("SELECT request_hash FROM processed_event WHERE event_id = ?", String.class, event.eventId()))) throw new IllegalArgumentException("Changed event identity");
            return;
        }
        ...
        if (added == 0) {
            if (!hash.equals(jdbc.queryForObject("SELECT request_hash FROM shipping WHERE order_id = ?", String.class, input.orderId()))) throw new IllegalArgumentException("Changed shipment instructions");
            return;
        }
        var output = new EventEnvelope<>(UUID.randomUUID(), failed ? "ShipmentFailed" : "ShipmentCreated", event.correlationId(), event.eventId(), "Shipment", input.orderId(),
                1, Instant.now(), 1, event.traceparent(), new ShippingEvents.Outcome(input.orderId(), input.customerId(), tracking, failed ? "UNSUPPORTED_DESTINATION" : null));
